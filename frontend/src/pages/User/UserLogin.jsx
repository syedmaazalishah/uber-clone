...
const UserLogin = () => {
	...
	async function handleFormSubmit () {
		...
		try {	
			setLoading( true ) ;
			const { data } = await axios.post( apisDictionary.UserLogin , loginData ) ;
			if ( data.success ) {
		...
		<section className='flex flex-col justify-between h-full' >
			...
			<div className="">
				...
				<form className='p-5' onSubmit={handleFormSubmit} >
					...
					<BigButton text='Login' IconBack={PersonStanding} onClick={ handleFormSubmit } label='Login' Icon={ ArrowRight } className='' />

					<p className="w-full text-center pt-2">
						<span className="">Wanna Enjoy Rides? </span>
			...
			<Link to='/captain-login' className='p-5' >
				<BigButton
					label='Captain Sign In'
					text='Sign In as Captain'
					Icon={ ArrowRight } IconBack={CarTaxiFront}
					className='bg-[#10b461]'
				/>
			</Link>
		</section>
